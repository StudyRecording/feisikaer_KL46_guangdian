...
const unsigned char draw_point[] = {0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80}; 

//const  unsigned char point[] = {~0x01,~0x02,~0x04,~0x08,~0x10,~0x20,~0x40,~0x80}; 
//======================================
...
0x00,0x10,0x10,0xFE,0x50,0x80,0x5C,0xAC,0xAC,0xAC,0xFC,0x44,0x40,0x00,
0x00,0x03,0x41,0x7F,0x42,0x26,0x1A,0x36,0x42,0x2E,0x11,0x2E,0x42,0x00,/*"摄",0*/
0x00,0x80,0x60,0xBC,0x22,0x70,0x5C,0x56,0xF4,0x5C,0x54,0xF0,0x00,0x00,
0x00,0x00,0x00,0x7F,0x50,0x2A,0x2A,0x55,0xCA,0x7E,0x0D,0x31,0x20,0x00,/*"像",1*/
0x00,0x00,0x20,0x44,0x88,0x18,0x00,0xFE,0x00,0x00,0x00,0x00,0x00,0x00,
0x00,0x82,0x42,0x42,0x22,0x12,0x0A,0x07,0x0A,0x12,0x12,0x22,0x42,0x00,/*"头",2*/
0x00,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x00,
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,/*"一",3*/
0x00,0x00,0xFC,0x44,0xB4,0x0C,0x00,0x00,0xFE,0xC0,0x00,0x00,0x00,0x00,
0x00,0x00,0x3F,0x08,0x58,0x2F,0x10,0x0E,0x01,0x03,0x0C,0x30,0x00,0x00,/*"队",4*/



0x00,0x08,0x08,0x08,0x18,0x28,0xCA,0xCC,0xB8,0x08,0x08,0x0C,0x08,0x00,
0x00,0x82,0x82,0x42,0x3D,0x03,0x00,0x00,0x00,0x7F,0x01,0x03,0x02,0x00,/*"齐",0*/
0x00,0x20,0x10,0xF8,0xB4,0xB6,0xF4,0xAC,0xB4,0xB4,0xF0,0x00,0x00,0x00,
0x00,0x00,0x02,0x7E,0x56,0x56,0x56,0x56,0x56,0x56,0x7E,0x01,0x01,0x00,/*"鲁",1*/
0x00,0x00,0x08,0x08,0x08,0x08,0xF8,0x08,0x08,0x08,0x08,0x0C,0x08,0x00,
0x00,0x00,0x20,0x20,0x20,0x20,0x3F,0x20,0x20,0x20,0x20,0x20,0x20,0x00,/*"工",2*/
0x00,0x20,0xC0,0x00,0x00,0xFC,0x00,0x00,0xFE,0x00,0x00,0xE0,0x20,0x00,
0x00,0x40,0x40,0x47,0x40,0x3F,0x40,0x40,0x3F,0x44,0x43,0x40,0x20,0x00,/*"业",3*/
...
0x00,0x00,0x90,0xFE,0x90,0x40,0x48,0x70,0x40,0x7E,0x60,0x58,0x80,0x00,
0x00,0x06,0x01,0x7F,0x01,0x00,0x24,0x24,0x24,0x24,0x24,0x24,0x3B,0x00,/*"档",0*/
0x00,0x80,0x60,0xFC,0x12,0x10,0x90,0x12,0x1C,0x10,0xD0,0x50,0x00,0x00,
0x00,0x01,0x00,0x7F,0x40,0x40,0x43,0x4C,0x40,0x3C,0x43,0x40,0x20,0x00,/*"位",1*/
...
///****************************************************************
//功能：OLED 写命令
//参数：cmd 命令内容，详见O-LED芯片(型号SSD1306 )手册
//返回：无
//详解：想OLED 写入想要显示的命令
//**************************************************************/
//
...
void LCD_Init(void)        
{
  ...
  spi_init(SPI1,SPI_PCS1, MASTER,10*1000*1000);
  LCD_RST=0;
  LCD_DLY_ms(50);
  LCD_RST=1;
  ...
  LCD_Fill(0x00);  //初始清屏
  LCD_Set_Pos(0,0);
//    LCD_P14x16Str(23,2,(byte *)"齐鲁工业大学"); 
//    LCD_P14x16Str(30,4,(byte *)"摄像头一队");       
...
//==============================================================
//函数名：Draw_point(unsigned char x,unsigned char y)
//功能描述：画点函数
//参数：显示的位置（x,y）
//返回：无
//==============================================================  
...
//==============================================================
//LCD_Print(byte x,byte y,byte ch[])
//功能描述：输出汉字和字符混合字符串
//参数：显示的位置（x,y），y为页范围0～7，要显示的字符串
//返回：无
//==============================================================  
...
/***************************************************************************
 功能：OLED显示一个一位数,可以显示正负数  主频100M耗时100us
 参数：显示的位置（x,y），shuju 为有符号一位数
 返回：无
 详解：可显示正负号
************************************************************************** */ 
void LCD_one(unsigned char x,unsigned char y,int16 shuju)
{
  ...
  for(i=0;i<6;i++)
	LCD_WrDat( F6x8[ temp+16 ][i] );  //个位  
}

/***************************************************************************
 功能：OLED显示一个两位数,可以显示正负数  主频100M耗时100us
 参数：显示的位置（x,y），shuju 为有符号两位数
 返回：无
 详解：可显示正负号
************************************************************************** */ 
...
LCD_Set_Pos(x+6,y);
	for(i=0;i<6;i++)
	LCD_WrDat( F6x8[ temp/10+16 ][i] );  //百位

	LCD_Set_Pos(x+12,y);
  for(i=0;i<6;i++)
	LCD_WrDat( F6x8[ temp%10+16 ][i] ); //个位 
}


/***************************************************************************
 功能：OLED显示一个三位数,可以显示正负数  主频100M耗时100us
 参数：显示的位置（x,y），shuju 为有符号三位数
 返回：无
 详解：可显示正负号
************************************************************************** */ 
void LCD_three(unsigned char x,unsigned char y,int16 shuju)
{
	...
	if(shuju<0)
	{
		...
		LCD_Set_Pos(x,y);
		for(i=0;i<6;i++)
		LCD_WrDat( F6x8[13][i] ); //负号
	}
	else
	{
		LCD_Set_Pos(x,y);
		for(i=0;i<6;i++)
		LCD_WrDat( F6x8[0][i] ); //正（不显示）
	}
	
	LCD_Set_Pos(x+6,y);
	for(i=0;i<6;i++)
	LCD_WrDat( F6x8[ temp/100+16 ][i] );  //百位

	LCD_Set_Pos(x+12,y);
  for(i=0;i<6;i++)
	LCD_WrDat( F6x8[ (temp/10)%10+16 ][i] );  //十位
...
/***************************************************************************
 功能：OLED显示一个四位数,可以显示正负数  主频100M耗时100us
 参数：显示的位置（x,y），shuju 为有符号四位数
 返回：无
 详解：可显示正负号
************************************************************************** */ 
void LCD_four(unsigned char x,unsigned char y,int16 shuju)
{
	...
	LCD_Set_Pos(x+6,y);
	for(i=0;i<6;i++)
	LCD_WrDat( F6x8[ temp/1000+16 ][i] ); //千位

	LCD_Set_Pos(x+12,y);
  for(i=0;i<6;i++)
	LCD_WrDat( F6x8[ (temp/100)%10+16 ][i] );   //百
	
	LCD_Set_Pos(x+18,y);
  for(i=0;i<6;i++)
	LCD_WrDat( F6x8[ temp/10%10+16 ][i] );  //十
...
/*****************************************************************************************************************
功能：显示 摄像头二值化图像
参数：a[HANG][LIE]  摄像头二值化数组
返回：无
详解：从下面宏定义改变其他参数
******************************************************************************************************************/
#define START_YE 1 //定义显示开始页数
#define START_LIE 47 //定义显示开始列数

void ShowVideoBin1(unsigned char a[HANG][LIE])
{
	uint8_t i = 0,j = 0,h = 0;
	uint8_t temp = 0;
	uint8_t ye = 0,ye_last = 0;
  uint8_t datatemp ;
	...
	for(h=0;h < ye-1;h++)
	{	
		LCD_Set_Pos(START_LIE,h+START_YE);//定义开始显示位
		for(j=0;j<LIE;j++)//打印一页数据
		{
			for(i = 0;i < 8;i ++)//竖着打印一个字节的数据
			{
				datatemp = a[i+8][j];
				datatemp <<=i;
				temp |=datatemp;
...
LCD_Set_Pos(START_LIE,h+START_YE);//定义开始显示位
	for(j=0;j<LIE;j++)//打印一页数据
		{
			for(i = 0;i < ye_last;i ++)//竖着打印一个字节的数据
			{
				datatemp = a[i+8][j];
				datatemp <<=i;
				temp |=datatemp;
...
void zhongxian()
{
 ...
 for(i=0;i<60;i++) 
 {
    ...
    zhong[i]=(40+M_err[i])/2;
    if(zhong[i]<0)
      zhong[i]=0;
        if(zhong[i]>40)
      zhong[i]=40;
    Draw_point(i,zhong[i]);
...
//  for(j=1;j<7;j++)//打印一页数据
//  {  
//    LCD_Set_Pos(a[8*j],j);//定义开始显示位
//    for(i = 0;i < 8;i ++)
//    {   
//      if(a[i+8*j]==a[8*j])
//      {
...
//  }
    ...
    //  uint8_t y = 0,j=0,temp = 0;
    //  for(y=8;y<56;y++)
    //  {
    //   
    //    LCD_Set_Pos(47+a[y],y/8);
    //    temp = 0;
    //    for(j=0;j<8;j++)
    //    {
    //      temp+=video[(y/8)*8+j][47+a[y]]<<j;    
    //    }      
    //    LCD_WrDat((point[y%8])&(~temp));
    //   
    // 
    //    
    //    
